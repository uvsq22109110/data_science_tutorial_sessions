vectorizer_ngrams = TfidfVectorizer(stop_words=english_stop_words,strip_accents="unicode",max_features=100,ngram_range=(2,4))
x_train_embedding_ngrams = vectorizer_ngrams.fit_transform(X_train)
print(vectorizer_ngrams.vocabulary_)
x_test_embedding_ngrams = vectorizer_ngrams.transform(X_test)
...
for clf_name in classifiers:
    ...
    best_model = clf.fit(x_train_embedding_ngrams,y_train)
...
mlp_Classifier = MLPClassifier(hidden_layer_sizes=(200, 120, 80), learning_rate= "constant", random_state=2020)
rf_classifier =  RandomForestClassifier(criterion= 'gini', max_depth= 50, n_estimators= 400, random_state = 2020)

mlp_Classifier.fit(x_train_embedding_ngrams,y_train)
rf_classifier.fit(x_train_embedding_ngrams,y_train)
